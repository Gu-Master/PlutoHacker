...
try:
    ...
    print(
        "You need Cython to build PlutoSDR Protocol Tool extensions.\n"
...
def set_builtin(name, value):
    if isinstance(__builtins__, dict):
        ...
        # support build frontends that expose __builtins__ as a module
...
def get_packages():
    ...
    for dirpath, dirnames, filenames in os.walk(os.path.join("./src/", PACKAGE_DIR)):
        package_path = os.path.relpath(
            dirpath, os.path.join("./src/", PACKAGE_DIR)
        ).replace(separator, ".")
        if len(package_path) > 1:
            packages.append(PACKAGE_DIR + "." + package_path)
...
setup(
    ...
    description="PlutoSDR-only protocol analysis tool",
    long_description=read_long_description(),
    long_description_content_type="text/markdown",
    author="Nikita Gurachevsky, IA-232",
    author_email="",
    package_dir={"": "src"},
    ...
    entry_points={
        "console_scripts": [
            "plutosdr_tool = urh.main:main",
            "plutosdr_cli = urh.cli.urh_cli:main",
        ]
    },
)
