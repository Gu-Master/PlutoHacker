...
class SendConfig(object):
    def __init__(
        self,
        send_buffer,
        current_sent_index: Value,
        current_sending_repeat: Value,
        total_samples: int,
        sending_repeats: int,
        continuous: bool = False,
        iq_to_bytes_method: callable = None,
        continuous_send_ring_buffer: RingBuffer = None,
    ):
    ...
    def get_data_to_send(self, buffer_length: int):
        try:
            ...
            if self.continuous:
                num_iq_samples = max(1, buffer_length // 2)
                iq_samples = self.continuous_send_ring_buffer.pop(num_iq_samples)
                if len(iq_samples) < num_iq_samples:
                    padded = np.zeros(
                        (num_iq_samples, 2), dtype=self.send_buffer._type_._type_
                    )
                    padded[: len(iq_samples)] = iq_samples
                    iq_samples = padded
                result = self.iq_to_bytes_method(iq_samples)
            else:
                index = self.current_sent_index.value
                np_view = np.frombuffer(
                    self.send_buffer, dtype=self.send_buffer._type_._type_
                )
    ...
    def sending_is_finished(self):
        ...
        return (
            self.current_sending_repeat.value >= self.sending_repeats
