...
class DeviceOptionsTableModel(QAbstractTableModel):
    ...
    def data(self, index: QModelIndex, role=Qt.ItemDataRole.DisplayRole):
        ...
        if role == Qt.ItemDataRole.DisplayRole:
            ...
            if j == 0 and device.has_native_backend:
                return (
    ...
    def flags(self, index: QModelIndex):
        ...
        if j == 0 and not device.has_native_backend:
            return Qt.ItemFlag.NoItemFlags

        if j in [1, 2] and not device.is_enabled:
            return Qt.ItemFlag.NoItemFlags
...
class OptionsDialog(QDialog):
    ...
    def __init__(self, installed_plugins, highlighted_plugins=None, parent=None):
        ...
        plugin_tab_index = self.ui.tabWidget.indexOf(self.ui.tab_plugins)
        if plugin_tab_index >= 0:
            self.ui.tabWidget.removeTab(plugin_tab_index)
        ...
        self.ui.labelDeviceMissingInfo.setText(
            self.tr(
                "Only the native PlutoSDR backend is used in this build. "
                "If PlutoSDR is missing, rebuild the native extension."
            )
        )

        self.ui.comboBoxTheme.setCurrentIndex(themes.read_theme_index())
    ...
    def show_native_backend_infos(self):
        self.backend_handler.set_native_backend_status()
    ...
    def refresh_device_tab(self):
        ...
        self.show_native_backend_infos()
        self.device_options_model.update()
    ...
    def set_native_backend_status(self):
        self.backend_handler.set_native_backend_status()
        self.refresh_device_tab()
        ...
        if rc == 0:
            self.ui.labelRebuildNativeStatus.setText(
                self.tr(
                    ...
                    "</font>"
                    "Please restart PlutoSDR Protocol Tool.".format(len(extensions))
                )
            )
        else:
            self.ui.labelRebuildNativeStatus.setText(
                self.tr(
                    ...
                    "</font>"
                    "Run PlutoSDR Protocol Tool as root (<b>sudo ./run_plutosdr.sh</b>) "
