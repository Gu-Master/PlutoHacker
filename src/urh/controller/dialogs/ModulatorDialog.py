...
class ModulatorDialog(QDialog):
    ...
    def mark_samples_in_view(self):
        ...
        if int(self.ui.gVOriginalSignal.view_rect().width()) != int(
            ...
            highlight_style = themes.text_color_stylesheet(themes.WARNING_COLOR)
            self.ui.lSamplesInViewOrigSignal.setStyleSheet(highlight_style)
            self.ui.lSamplesInViewModulated.setStyleSheet(highlight_style)
        else:
            font = self.ui.lSamplesInViewModulated.font()
            font.setBold(True)
            self.ui.lSamplesInViewModulated.setFont(font)
