...
class CompareFrameController(QWidget):
    ...
    def __init__(
        ...
        self.ui.lBits.hide()
        self.ui.lBitsSelection.hide()
        self.ui.lHex.hide()
        self.ui.lHexSelection.hide()
        self.ui.label_5.setVisible(False)
        self.ui.cbDecoding.setVisible(False)
        self.ui.lEncodingErrors.setVisible(False)
        self.ui.lDecodingErrorsValue.setVisible(False)
    ...
    def __localize_ui(self):
        ...
        self.ui.btnAnalyze.setText(self.tr("Упростить анализ"))
        self.ui.btnAnalyze.setToolTip(
            self.tr("Автоматически разобрать структуру протокола.")
        )
    ...
    def __set_decoding_error_label(self, message: Message):
        if message:
            ...
            color = (
                themes.SUCCESS_COLOR
                if errors == 0 and state == ""
                else themes.ERROR_COLOR
            )
            self.ui.lDecodingErrorsValue.setStyleSheet(
                themes.text_color_stylesheet(color)
            )
