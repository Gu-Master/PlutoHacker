...
class DeviceSettingsWidget(QWidget):
    ...
    def __init__(
        ...
        self.__apply_single_device_mode()
    ...
    def __localize_ui(self):
        self.ui.groupBoxDeviceSettings.setTitle(self.tr("Параметры устройства"))
        self.ui.label_3.setText(self.tr("Устройство:"))
        self.ui.labelDeviceIdentifier.setText(self.tr("Идентификатор устройства:"))
        self.ui.labelSubdevice.setText(self.tr("Подустройство:"))
        self.ui.labelChannel.setText(self.tr("Канал:"))
        self.ui.labelAntenna.setText(self.tr("Антенна:"))
        self.ui.labelIP.setText(self.tr("IP-адрес:"))
        self.ui.labelPort.setText(self.tr("Порт:"))
        self.ui.labelFreq.setText(self.tr("Частота (Гц):"))
        self.ui.labelSampleRate.setText(self.tr("Частота дискретизации (Sps):"))
        self.ui.labelBandwidth.setText(self.tr("Полоса (Гц):"))
        self.ui.labelGain.setText(self.tr("Усиление:"))
        self.ui.labelIFGain.setText(self.tr("IF усиление:"))
        self.ui.labelBasebandGain.setText(self.tr("Baseband усиление:"))
        self.ui.labelFreqCorrection.setText(self.tr("Коррекция частоты:"))
        self.ui.labelDirectSampling.setText(self.tr("Прямое семплирование:"))
        self.ui.labelNRepeat.setText(self.tr("Повторов:"))
        self.ui.labelDCCorrection.setText(self.tr("DC коррекция:"))
        self.ui.labelBiasTee.setText(self.tr("Bias Tee:"))
        self.ui.checkBoxDCCorrection.setText(self.tr("Включить DC коррекцию"))
        self.ui.checkBoxBiasTee.setText(self.tr("Включить Bias Tee"))
        self.ui.spinBoxNRepeat.setSpecialValueText(self.tr("Бесконечно"))

    def __apply_single_device_mode(self):
        single_device = self.ui.cbDevice.count() <= 1
        self.ui.label_3.setVisible(not single_device)
        self.ui.cbDevice.setVisible(not single_device)
    ...
    def bootstrap(self, conf_dict: dict, enforce_default=False):
        ...
        set_val(
            self.ui.spinBoxIFGain, self.rx_tx_prefix + "if_gain", config.DEFAULT_IF_GAIN
        )
        set_val(
            self.ui.spinBoxBasebandGain,
            self.rx_tx_prefix + "baseband_gain",
            config.DEFAULT_BB_GAIN,
        )
        set_val(
            self.ui.spinBoxFreqCorrection,
            "freq_correction",
    ...
    def create_connects(self):
        self.ui.spinBoxFreq.editingFinished.connect(
        ...
        self.ui.spinBoxGain.editingFinished.connect(
            self.on_spinbox_gain_editing_finished
        )
        ...
        self.ui.spinBoxIFGain.editingFinished.connect(
            self.on_spinbox_if_gain_editing_finished
        )
        self.ui.spinBoxIFGain.valueChanged.connect(
            self.on_spinbox_if_gain_value_changed
        )
        ...
        self.ui.comboBoxDeviceIdentifier.editTextChanged.connect(
            self.on_combo_box_device_identifier_edit_text_changed
        )

        self.ui.checkBoxBiasTee.clicked.connect(self.on_check_box_bias_tee_clicked)
        self.ui.checkBoxDCCorrection.clicked.connect(
            self.on_check_box_dc_correction_clicked
        )
    ...
    def set_default_bb_gain(self):
        ...
        if prefix + "baseband_gain" in conf:
            ...
            baseband_gain = conf[key][
                int(np.percentile(list(range(len(conf[key]))), 25))
            ]
    ...
    def sync_gain_sliders(self):
        ...
        self.ui.spinBoxBasebandGain.valueChanged.emit(
    ...
    def set_device_ui_items_visibility(self, device_name: str, overwrite_settings=True):
        ...
        if "direct_sampling" in conf:
            ...
            items = [
                self.ui.comboBoxDirectSampling.itemText(i)
                for i in range(self.ui.comboBoxDirectSampling.count())
            ]
        ...
        multi_dev_support = (
            hasattr(self.device, "has_multi_device_support")
            and self.device.has_multi_device_support
        )
        ...
        if (
            ...
            if not self.device.bandwidth_is_adjustable:
                ...
                self.ui.spinBoxBandwidth.setToolTip(
                    self.tr(
                        "The active PlutoSDR backend does not report adjustable "
                        "bandwidth support."
                    )
                )
            else:
    ...
    def emit_editing_finished_signals(self):
        ...
        self.ui.comboBoxAntenna.currentIndexChanged.emit(
            self.ui.comboBoxAntenna.currentIndex()
        )
        self.ui.comboBoxChannel.currentIndexChanged.emit(
            self.ui.comboBoxChannel.currentIndex()
        )
        self.ui.checkBoxDCCorrection.clicked.emit(
            self.ui.checkBoxDCCorrection.isChecked()
        )
    ...
    def _emit_live_device_parameters(self):
        if self.device is not None:
            self.emit_device_parameters_changed()
    ...
    def on_spinbox_sample_rate_editing_finished(self):
        ...
        self._emit_live_device_parameters()
    ...
    def on_spinbox_gain_value_changed(self, value: int):
        ...
        try:
            self.ui.sliderGain.setValue(
                dev_conf[self.rx_tx_prefix + "rf_gain"].index(value)
            )
    ...
    def on_spinbox_if_gain_value_changed(self, value: int):
        ...
        try:
            self.ui.sliderIFGain.setValue(
                dev_conf[self.rx_tx_prefix + "if_gain"].index(value)
            )
        except (ValueError, KeyError):
            pass
    ...
    def on_slider_baseband_gain_value_changed(self, value: int):
        ...
        self.ui.spinBoxBasebandGain.setValue(
            dev_conf[self.rx_tx_prefix + "baseband_gain"][value]
        )
    ...
    def update_for_new_device(self, overwrite_settings=True):
        ...
        dev_name = self.ui.cbDevice.currentText()
        self.set_device_ui_items_visibility(
            dev_name, overwrite_settings=overwrite_settings
        )
