...
DEFAULT_SIGNAL_FOLDER_NAME = "PlutoSDR_Signals"
...
def __get__name_filter_for_signals() -> str:
    return ";;".join(
        ...
        + SIGNAL_NAME_FILTERS
        + [COMPRESSED_COMPLEX_FILE_FILTER, WAV_FILE_FILTER]
    )


def get_default_signal_directory(create=True) -> str:
    default_path = settings.read(
        "default_signal_directory",
        os.path.join(QDir.homePath(), DEFAULT_SIGNAL_FOLDER_NAME),
        str,
    )
...
def set_default_signal_directory(path: str):
    if not path:
        return

    path = os.path.abspath(os.path.expanduser(path))
    os.makedirs(path, exist_ok=True)
    settings.write("default_signal_directory", path)


def build_default_signal_path(filename: str) -> str:
    directory = get_default_signal_directory(create=True)
...
def save_signal_data_to_default_directory(
    signal_name: str, data, sample_rate=1e6, wav_only=False
) -> str:
    global RECENT_PATH
    if wav_only:
        if not signal_name.endswith(".wav") and not signal_name.endswith(".wave"):
        ...
        if not any(
            signal_name.endswith(e) for e in SIGNAL_FILE_EXTENSIONS_BY_TYPE.values()
        ):
            try:
                dtype = next(
                    d for d in SIGNAL_FILE_EXTENSIONS_BY_TYPE.keys() if d == data.dtype
    ...
    filename = build_default_signal_path(signal_name)
    ...
    set_default_signal_directory(RECENT_PATH)
    return filename
