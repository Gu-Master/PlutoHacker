...
def get_shared_library_path():
    for env_name in ("PLUTOSDR_SHARED_LIB_DIR", "IIO_DLL_DIR", "LIBIIO_DLL_DIR"):
        shared_lib_dir = os.getenv(env_name, "").strip()
        if shared_lib_dir and os.path.isdir(shared_lib_dir):
            return os.path.realpath(shared_lib_dir)
    ...
    for env_name in ("IIO_ROOT", "LIBIIO_ROOT", "PLUTOSDR_IIO_ROOT"):
        ...
        if sys.platform == "win32":
            shared_lib_dir = _find_parent_dir_with_file(root, ("iio.dll",))
        elif sys.platform == "darwin":
            shared_lib_dir = _find_parent_dir_with_file(root, ("libiio.dylib",))
        else:
            shared_lib_dir = _find_parent_dir_with_file(
