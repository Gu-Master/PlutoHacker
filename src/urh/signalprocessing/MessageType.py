...
class MessageType(list):
    ...
    def get_first_label_with_type(
        self, field_type: FieldType.Function
    ) -> ProtocolLabel:
        return next(
            (
                lbl
                for lbl in self
                if lbl.field_type and lbl.field_type.function == field_type
            ),
            None,
        )
    ...
    def add_protocol_label(
        ...
        proto_label = self.__create_label(
            name=name,
            start=start,
            end=end,
    ...
    def add_protocol_label_start_length(
        self,
        start: int,
        length: int,
        name=None,
        ...
        return self.add_protocol_label(
            start, start + length - 1, name, color_ind, auto_created, type
        )
